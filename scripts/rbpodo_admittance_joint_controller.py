...
"""
Function-call style task-space admittance + IK + joint servo-j controller.

Compared to topic version:
- No target_pose subscriber
- No periodic timer
- Caller sets target pose and calls step_once() explicitly
"""
...
class RBPodoAdmittanceJointControllerFn(RB10Controller):
    def __init__(self) -> None:
        ...
        self.calibrate_bias(self._auto_bias_samples)

        self.get_logger().info(f"Function-call admittance+IK controller started (hz={self._hz:.1f})")
    ...
    def set_target_pose(self, pos_xyz, quat_xyzw) -> None:
        self._x_r = np.asarray(pos_xyz, dtype=float).reshape(3,)
        self._q_r = _normalize_quat(np.asarray(quat_xyzw, dtype=float).reshape(4,))

    def hold_current(self) -> bool:
        pose = self.get_current_ee_pose()
        if pose is None:
            return False
        p, q = pose
    ...
    def calibrate_bias(self, samples: int = 100) -> bool:
    ...
    def run_for(self, sec: float) -> None:
        ...
        dt = self._dt
        while rclpy.ok() and time.perf_counter() < end_t:
            t0 = time.perf_counter()
            self.step_once()
            sleep_sec = dt - (time.perf_counter() - t0)
            if sleep_sec > 0.0:
...
def main() -> None:
    rclpy.init()
    node = RBPodoAdmittanceJointControllerFn()
    try:
        ok = node.hold_current()
        if not ok:
            node.get_logger().warn("Failed to hold current pose.")
        node.run_for(10.0)
    finally:
        node.destroy_node()
        rclpy.shutdown()
