...
- BASE(frame) 기준 EE pose (pos[3] + quat[xyzw]) -> IK -> rbpodo move_j publish
...
RBPODO_MIN_CMD_DT = CONTROL_DT
RBPODO_DATA_TIMEOUT = 0.05
RBPODO_CMD_SPEED = 20.0  # deg/sm (5.0~250.0 권장)
RBPODO_CMD_ACCEL = 40.0  # deg/s^2 (10.0~500.0 권장)
RBPODO_WAIT = False
...
class RB10Controller(Node):
    """ BASE(frame) 기준 pos(3) + quat(xyzw) -> IK -> rbpodo move_j publish """
    def __init__(
        self,
    ...
    def publish_qpos(
        ...
        if self._last_cmd_ns > 0:
            dt = (now_ns - self._last_cmd_ns) * 1e-9
            if dt < RBPODO_MIN_CMD_DT:
                return True  # throttle overspeed command bursts
        self._last_cmd_ns = now_ns
        return self._send_qpos(q_goal=q_goal, speed=speed, accel=accel)

    def publish_qpos_trajectory(
        self,
        ...
        for i, q_goal in enumerate(q_traj):
            ok = self._send_qpos(
                q_goal=q_goal,
                speed=speed,
                accel=accel,
            ...
            if not ok and stop_on_fail:
                self.get_logger().warn(f"Trajectory aborted at waypoint {i}")
                return False
            if i < len(q_traj) - 1:
                # Absolute-time scheduler: keeps near-constant control cycle with less drift.
                next_tick = t0 + (i + 1) * CONTROL_DT
                sleep_sec = next_tick - time.perf_counter()
                if sleep_sec > 0.0:
                    time.sleep(sleep_sec)
                elif DEBUG:
                    self.get_logger().warn(
                        f"Trajectory tick overrun at waypoint {i} ({-sleep_sec * 1000.0:.1f} ms)"
                    )
        return True
    ...
    def _send_qpos(
        ...
        try:
            ...
            # Fixed speed/accel policy by default.
            cmd_speed = RBPODO_CMD_SPEED if speed is None else float(speed)
            cmd_accel = RBPODO_CMD_ACCEL if accel is None else float(accel)
            cmd_speed = float(np.clip(cmd_speed, 5.0, 250.0))
            cmd_accel = float(np.clip(cmd_accel, 10.0, 500.0))

            self._rb_robot.move_j(self._rb_rc, q_goal_deg, cmd_speed, cmd_accel, int(1 if RBPODO_WAIT else 0))
            return True
        except Exception as e:
            self.get_logger().warn(f"rbpodo move_j failed: {e}")
            return False
