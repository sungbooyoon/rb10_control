...
Robomimic-style RGB handling (OOM-safe):
- RGB stored as uint8 in HDF5 with chunking (1, H, W, 3) + gzip
- RGB is NOT accumulated in RAM (no np.stack over the whole trajectory)
- RGB is streamed directly from rosbag to HDF5 per demo using ref_ts alignment

Conventions:
- Each stroke segment (start~end) from /rb/stroke_event becomes one demo_* trajectory.
- OBS = separate ee_pos (m) and ee_quat (xyzw).  next_obs is not stored.
- ACTION = Δee_pos (3) + 6D relative rotation (first two columns of R_rel) -> (N,9)
- Optional per-demo normalization to [-1, 1] using 99th percentile or user-provided scales.

Requirements:
- rosbag2_py, rosidl_runtime_py, rclpy (for deserialize_message)
...
DEFAULT_TOPICS = {
    ...
    "stroke_event": "/rb/stroke_event",               # std_msgs/msg/String (JSON)
}
...
def extract_segments_from_stroke_events(stroke_timed_msgs: List[Tuple[float, object]]) -> List[Dict]:
    ...
    Output dict: {skill_id, stroke_id, t_start_sec, t_end_sec, t_start_ns, t_end_ns}
    """
    segs: List[Dict] = []
...
def slice_demo_by_time_window(demo: Dict, t_start: float, t_end: float) -> Optional[Dict]:
    """Slice a demo dict to [t_start, t_end] based on demo['meta']['ref_ts'] seconds."""
    ...
    out = {
        ...
        "dones": np.concatenate(
            [np.zeros((int(np.count_nonzero(keep)) - 1,), dtype=np.uint8), np.array([1], dtype=np.uint8)], axis=0
        ),
        "obs": {
            ...
            "rgb": None,  # streaming write later
        },
        "meta": dict(demo["meta"]),
    }
...
class BagReader:
    ...
    def read_all(self, wanted_topics: List[str]) -> Dict[str, List[Tuple[float, object]]]:
        """
        WARNING: This accumulates messages in RAM. Use only for non-image topics.
        """
...
# ---------------- Streaming RGB writer (robomimic-style) ----------------
def _choose_nearest(prev_t, prev_msg, cur_t, cur_msg, ref_t):
    best = None
    if prev_msg is not None:
        best = (prev_t, prev_msg)
    if cur_msg is not None:
        if best is None:
            best = (cur_t, cur_msg)
        else:
            if abs(cur_t - ref_t) < abs(best[0] - ref_t):
                best = (cur_t, cur_msg)
    return best


def stream_rgb_to_hdf5(
    bag_uri: str,
    rgb_topic: str,
    ref_ts: np.ndarray,  # (N,) seconds
    sync_tol: float,
    image_resize: Optional[Tuple[int, int]],
    dset_rgb,  # h5py dataset (N,H,W,3) uint8
):
    """
    Stream-read only rgb_topic from bag and write frames aligned to ref_ts into dset_rgb.
    - No accumulation / no np.stack
    - Carry-forward last valid image when missing
    - Backfill leading missing frames with first valid
    """
    # Open bag just to get msg type
    try:
        tmp = BagReader(bag_uri)
    except Exception as e:
        print(f"[WARN] Failed to open bag for RGB streaming: {bag_uri} ({e})", file=sys.stderr)
        dset_rgb[...] = 0
        return

    if rgb_topic not in tmp.topic_map:
        dset_rgb[...] = 0
        return

    info = tmp.topic_map[rgb_topic]

    rr = rosbag2_py.SequentialReader()
    rr.open(
    ...
    def next_rgb():
        while rr.has_next():
            topic, data, t_ns = rr.read_next()
            if topic != rgb_topic:
                continue
            msg = deserialize_message(data, info.msg_type)
            return ns_to_sec(t_ns), msg
        return None, None

    last_img = None
    leading_missing = []
    prev_t, prev_msg = None, None
    cur_t, cur_msg = next_rgb()

    for i, ref_t in enumerate(ref_ts.tolist()):
        while cur_msg is not None and cur_t < ref_t:
            prev_t, prev_msg = cur_t, cur_msg
            cur_t, cur_msg = next_rgb()

        chosen = _choose_nearest(prev_t, prev_msg, cur_t, cur_msg, ref_t)
        if chosen is None:
            if last_img is None:
                leading_missing.append(i)
            else:
                dset_rgb[i] = last_img
            continue

        t_sel, msg_sel = chosen
        if abs(t_sel - ref_t) > sync_tol:
            if last_img is None:
                leading_missing.append(i)
            else:
                dset_rgb[i] = last_img
        ...
        try:
            arr = rosimg_to_rgb_numpy(msg_sel)
            arr = resize_if_needed(arr, image_resize)
            arr = arr.astype(np.uint8, copy=False)
            last_img = arr
            dset_rgb[i] = last_img
        except Exception as e:
            # decoding failure -> carry-forward
            if last_img is None:
                leading_missing.append(i)
            else:
                dset_rgb[i] = last_img

    if leading_missing and last_img is not None:
        for idx in leading_missing:
            dset_rgb[idx] = last_img
    elif leading_missing and last_img is None:
        for idx in leading_missing:
            dset_rgb[idx] = 0
...
def process_single_bag(
    ...
    """
    Returns:
      {"demo": base_demo, "segments": segments}
    Note: base_demo has rgb=None always. RGB will be streamed at write time.
    """
    try:
    ...
    required = [topics["joint_states"], topics["ee_pose"], topics["stroke_event"]]
    for rt in required:
        if rt is None or rt not in reader.topic_map:
    ...
    if topics.get("ee_wrench") and topics["ee_wrench"] in reader.topic_map:
        wanted.append(topics["ee_wrench"])
    # IMPORTANT: do NOT read RGB via read_all (OOM). We'll stream later.
    data = reader.read_all(wanted)

    # reference timeline from ee_pose
    ...
    # joint_states aligned to ref_ts
    ...
    for i in range(len(ref_ts)):
        ...
        pos[i] = [p.x, p.y, p.z]
        quat_xyzw[i] = np.array([q_wxyz[1], q_wxyz[2], q_wxyz[3], q_wxyz[0]], dtype=np.float64)

    # wrench
    ee_wrench = None
    if topics.get("ee_wrench") and topics["ee_wrench"] in data and len(data[topics["ee_wrench"]]) > 0:
    ...
    # ----- actions = Δpos(3) + 6D rot(rel) -> 9D -----
    def compute_action_ee_delta_6d(
        pos_: np.ndarray, quat_: np.ndarray, normalize: bool, given_scale9: Optional[np.ndarray]
    ) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        N = pos_.shape[0]
        dpos = np.zeros((N, 3), dtype=np.float64)
        r6d = np.zeros((N, 6), dtype=np.float64)
        if N >= 2:
            dpos[:-1] = pos_[1:] - pos_[:-1]
            for i in range(N - 1):
                q_rel = quat_relative_xyzw(quat_[i + 1], quat_[i])
                R_rel = quat_to_rotmat_xyzw(q_rel)
                r6d[i] = rotmat_to_6d(R_rel)
            dpos[-1] = dpos[-2]
            r6d[-1] = r6d[-2]

        actions = np.concatenate([dpos, r6d], axis=1)
        if not normalize:
    ...
    actions9, action_scale = compute_action_ee_delta_6d(
        pos_=pos, quat_=quat_xyzw, normalize=normalize_actions, given_scale9=given_scale
    )
    ...
    demo = {
        "N": len(ref_ts),
        "actions": actions9.astype(np.float32),
        "rewards": np.zeros((len(ref_ts),), dtype=np.float32),
        "dones": np.concatenate(
            [np.zeros((len(ref_ts) - 1,), dtype=np.uint8), np.array([1], dtype=np.uint8)], axis=0
        ),
        "obs": {
            "joint_pos": q.astype(np.float32),
            "joint_vel": qd.astype(np.float32),
            "ee_pos": pos.astype(np.float32),
            "ee_quat": quat_xyzw.astype(np.float32),
...
def write_many_demos(
    ...
    """
    Write demos into a robomimic HDF5.
    If use_rgb: stream RGB per demo directly from the corresponding bag -> HDF5 (uint8, chunked).
    """
    if len(demos) == 0:
    ...
    with h5py.File(out_path, "w") as f:
        ...
        for k, demo in iterator:
            ...
            go.create_dataset("ee_pos", data=demo["obs"]["ee_pos"])
            go.create_dataset("ee_quat", data=demo["obs"]["ee_quat"])
            if demo["obs"]["ee_wrench"] is not None:
                go.create_dataset("ee_wrench", data=demo["obs"]["ee_wrench"])

            # --- RGB: streaming write (uint8 + chunk) ---
            if use_rgb:
                rgb_topic = demo["meta"].get("rgb_topic", None)
                ref_ts = np.array(demo["meta"].get("ref_ts", []), dtype=np.float64)

                if image_resize is None:
                    print(
                        f"[WARN] {name}: --rgb enabled but --image-resize not set. "
                        f"High-res RGB can make HDF5 huge (though streaming avoids OOM). "
                        f"Strongly recommended to set --image-resize.",
                        file=sys.stderr,
                    )

                if rgb_topic and ref_ts.size == N and image_resize is not None:
                    H, W = image_resize
                    dset = go.create_dataset(
                        "rgb",
                        shape=(N, H, W, 3),
                        dtype=np.uint8,
                        chunks=(1, H, W, 3),
                    ...
                    stream_rgb_to_hdf5(
                        bag_uri=demo["meta"]["bag_uri"],
                        rgb_topic=rgb_topic,
                        ref_ts=ref_ts,
                        sync_tol=float(demo["meta"].get("sync_tol", 0.05)),
                        image_resize=image_resize,
                        dset_rgb=dset,
                    )
                elif rgb_topic and ref_ts.size == N and image_resize is None:
                    # We could infer H,W by scanning first frame, but that still requires reading.
                    # Keep explicit: require resize for RGB writes in this script.
                    print(
                        f"[WARN] {name}: RGB topic present but --image-resize missing -> skipping RGB write for safety.",
                        file=sys.stderr,
                    )
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=None,
        metavar=("H", "W"),
        help="Resize RGB to (H W); omit to skip RGB write unless you modify inference logic",
    )
    ...
    g_rgb.add_argument("--rgb", dest="use_rgb", action="store_true",
                       help="Write RGB images into the HDF5 (streaming; still recommend --image-resize).")
    g_rgb.add_argument("--no-rgb", dest="use_rgb", action="store_false",
                       help="Do not store RGB images (default).")
...
def main():
    ...
    for uri in uris:
        ...
        if args.use_rgb and not args.image_resize:
            print("[WARN] --rgb enabled without --image-resize. This script will skip RGB writing for safety.", file=sys.stderr)
    ...
    write_many_demos(args.out, demos, use_rgb=args.use_rgb, image_resize=image_resize)
