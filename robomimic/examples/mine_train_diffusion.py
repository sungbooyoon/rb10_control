...
def robosuite_hyperparameters(config):
    ...
    config.experiment.save.every_n_epochs = 100                  # save model every n epochs (set to None to disable)
...
def get_config(dataset_type="robosuite", dataset_path=None, output_dir=None, debug=False):
    ...
    ### Experiment Config ###
    config.experiment.name = f"{dataset_type}_diffusion_example"   # name of experiment used to make log files
    config.experiment.validate = False                           # whether to do validation or not
    config.experiment.logging.terminal_output_to_txt = False    # whether to log stdout to txt file 
    config.experiment.logging.log_wandb = True                     # enable tensorboard logging
